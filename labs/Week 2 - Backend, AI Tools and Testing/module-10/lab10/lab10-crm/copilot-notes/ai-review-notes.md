...
1. Real customer data avoided in Chat: never typed anything resembling a real name,
   ...
   Khan / Ravi Singh), which are this bootcamp's fixed fictional examples.
2. If a suggestion looks copied verbatim from a known library/article: don't accept it
   blind — reread it like unfamiliar code from a coworker, and if it still looks
   ...
   pasting it as-is.
3. Team rule for code you don't fully understand: don't merge it. Full stop — same bar
   as if a human teammate handed you code and couldn't explain it either.
